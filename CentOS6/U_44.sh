U_44() {
    ...
    if [ -f /etc/passwd ]; then
        ...
        if [ ${#user_with_uid_zero[@]} -eq 0 ]; then
            echo -en "[양호]\t" >> "$rf" 2>&1
            echo "root 계정을 제외한 로그인이 가능한 모든 사용자 UID값이 '0'으로 설정되어 있지 않은 상태입니다."  >> "$rf" 2>&1
        else
            echo -en "[취약]\t" >> "$rf" 2>&1
            echo -en "root 계정과 동일한 UID(0)를 갖는 사용자가 존재합니다\t"  >> "$rf" 2>&1
            echo -en "주요통신기반시설 가이드를 참고하시어 " >> "$rf" 2>&1
            for ((i=0; i<${#user_with_uid_zero[@]}; i++)); do
                echo -en "${user_with_uid_zero[i]}" >> "$rf" 2>&1
            done
            echo " 의 UID값을 변경하여 주시기 바랍니다." >> "$rf" 2>&1
        fi
    fi
}
U_44
